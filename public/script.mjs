...
import {
 codeEditor
} from './components/code-editor/code-editor.mjs';
import {
 log
} from './components/log/log.mjs';

import {
 lessons
} from './components/lessons/lessons.mjs';
...
`);
 ...
 let n = 0;
 const logoAnim = setInterval(function() {
  $('#logo').css('filter', `invert(${Math.sin(n)*50+50}%)`);
  if (++n > 1000) clearInterval(logoAnim);
 }, 50);
...
IDE.save = function() {
 ...
 IDE.socket.emit('save', {
  path: IDE.currentLesson.saveTarget,
  txt: editor.getValue()
 }, res => {
  if (res == 'success') {
   $('.save-btn').css('filter', 'invert(100%)');
   ...
   if (IDE.currentLesson.saveTarget.includes('socket/addons')) {
    IDE.socket.emit('refresh', res => {
     if (res == 'success') {
      ...
      alert('fail in refreshing server..')
     }
    });
